...
// Tipos (importados desde los hooks)
...
import type { SolicitudCompra, CategoriaGeneral } from "../hooks/useRegistrarCompra";

// Nuevo formulario para generar PDF de pago
...
interface ComprasHook {
    isModalOpen: boolean;
    vista: 'lista' | 'formulario';
    solicitudesVisibles: SolicitudCompra[];
    alertasVencimiento: SolicitudCompra[];
    tipoSeleccionado: CategoriaGeneral;
    cerrarModal: () => void;
    cambiarVista: (vista: 'lista' | 'formulario') => void;
    crearSolicitud: (datos: Omit<SolicitudCompra, 'id' | 'estado' | 'ejecutada' | 'eliminada' | 'fecha_creacion' | 'hora_creacion'>) => void;
...
export const AdminModales = ({ 
    ...
    pagos, trabajo, trabajadores, compras, generarPDF
}: Props) => {
    return (
        <>
            {/* ── GANADO ── */}
            <ModalGenerico
                isOpen={ganado.isModalOpen}
            ...
            >
                {ganado.vista === 'lista' ? (
                        ...
                        <div className="overflow-hidden rounded-xl border border-gray-100 shadow-sm">
                            <table className="w-full text-left text-[11px] uppercase tracking-wider">
                                ...
                                <tbody className="text-gray-600">
                                    {ganado.listaGanado?.length > 0 ? (
                                        ganado.listaGanado.map((animal: any) => (
                                            <tr key={animal.id} className="border-t border-gray-50 hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="p-3">
                                                    <span className="bg-emerald-100 text-emerald-600 px-2 py-1 rounded-full text-[9px]">{animal.estado}</span>
                                                </td>
                                            </tr>
                                        ))
                                    ) : (
                    ...
                    <FormularioGanado
                        ...
                        onGuardar={(animal: any) => ganado.guardarAnimal(animal, true)}
                    />
                )}
            </ModalGenerico>
            ...
            >
                {cerdos.vista === 'lista' ? (
                            ...
                            <table className="w-full text-left text-[11px] uppercase">
                                ...
                                <tbody>
                                    {cerdos.listaCerdos?.length > 0 ? (
                                        cerdos.listaCerdos.map((cerdo: any) => (
                                            <tr key={cerdo.id} className="border-t border-emerald-50">
                                                ...
                                                <td className="p-3">
                                                    <span className="bg-emerald-100 text-emerald-600 px-2 py-1 rounded-full">{cerdo.estado}</span>
                                                </td>
                                            </tr>
                                        ))
                                    ) : (
                                        <tr>
                                            <td colSpan={3} className="p-6 text-center text-gray-300 text-[11px] uppercase font-bold italic">
                                                — Sin registros aún —
                                            </td>
                                        </tr>
                                    )}
                    ...
                    <FormularioCerdo
                        ...
                        categoriaSeleccionada={cerdos.categoriaCerdo}
                    ...
                    <div className="flex flex-col gap-6 p-4">
                        <div className="overflow-hidden rounded-xl border border-cyan-100 shadow-sm">
                            <table className="w-full text-left text-[11px] uppercase">
                                ...
                                <tbody className="text-gray-600">
                                    {vacunas.listaVacunas?.length > 0 ? (
                                        vacunas.listaVacunas.map((v: any) => (
                    ...
                    <div className="flex flex-col gap-6 p-4">
                        <div className="overflow-hidden rounded-xl border border-orange-100 shadow-sm">
                            <table className="w-full text-left text-[11px] uppercase">
                                ...
                                <tbody className="text-gray-600">
                                    {ventas.listaVentas?.length > 0 ? (
                                        ventas.listaVentas.map((v: any) => (
            ...
            {/* ══════════════════════════════════════ */}
            {/* MÓDULOS DE PERSONAL — COMPLETADOS Y CORREGIDOS */}
            {/* ══════════════════════════════════════ */}

            {/* ── PAGOS ── */}
            <ModalGenerico
                isOpen={pagos.isModalOpen}
            ...
            >
                {pagos.vista === 'lista' ? (
                                            ...
                                            <tr key={p.id} className="border-t border-purple-50">
                                                ...
                                                <td className="p-3">
                                                    <span className={`px-2 py-1 rounded-full text-[9px] ${
                                                        p.estado === 'Pagado con firma' ? 'bg-green-100 text-green-600' : 
                                                        p.estado === 'Pendiente de firma' ? 'bg-yellow-100 text-yellow-600' :
                    ...
                    <FormularioPagos
                        pagoAEditar={pagos.pagoAEditar}
                        onGuardar={(datos, cerrar) => pagos.guardarPago(datos, cerrar)}
            ...
            >
                {trabajo.vista === 'lista' ? (
                    <div className="flex flex-col gap-6 p-4">
                        <div className="overflow-hidden rounded-xl border border-blue-100 shadow-sm">
                            <table className="w-full text-left text-[11px] uppercase">
                                ...
                                <tbody>
                                    {trabajo.trabajosActivos?.length > 0 ? (
                                        trabajo.trabajosActivos.map((t) => (
                    ...
                    <FormularioTrabajoRealizado
                        trabajoAEditar={trabajo.trabajoAEditar}
                        onGuardar={(datos, cerrar) => trabajo.guardarTrabajo(datos, cerrar)}
            ...
            {/* ── NUEVO TRABAJADOR ── */}
            <ModalGenerico
                isOpen={trabajadores.isModalOpen}
            ...
            >
                {trabajadores.vista === 'lista' ? (
                    <div className="flex flex-col gap-6 p-4">
                        <div className="overflow-hidden rounded-xl border border-indigo-100 shadow-sm">
                            <table className="w-full text-left text-[11px] uppercase">
                                ...
                                <tbody>
                                    {trabajadores.trabajadoresVisibles?.length > 0 ? (
                                        trabajadores.trabajadoresVisibles.map((t) => (
                    ...
                    <FormularioNuevoTrabajador
                        trabajadorAEditar={trabajadores.trabajadorAEditar}
                        onGuardar={(datos, cerrar) => trabajadores.guardarTrabajador(datos, cerrar)}
            ...
            >
                {compras.vista === 'lista' ? (
                    <div className="flex flex-col gap-6 p-4">
                        ...
                        <div className="overflow-hidden rounded-xl border border-amber-100 shadow-sm">
                            <table className="w-full text-left text-[11px] uppercase">
                                ...
                                <tbody>
                                    {compras.solicitudesVisibles?.length > 0 ? (
                                        compras.solicitudesVisibles.map((s) => (
                    ...
                    <FormularioCompra
                        tipoSeleccionado={compras.tipoSeleccionado}
                        setTipoSeleccionado={compras.setTipoSeleccionado}
                        onGuardar={compras.crearSolicitud}
                    ...
                    />
                        ...
                        {/* ══════════════════════════════════════ */}
            {/* NUEVO MÓDULO: GENERAR FORMATO PAGO PDF */}
            {/* ══════════════════════════════════════ */}

            {/* ── GENERAR FORMATO PAGO PDF ── */}
            <ModalGenerico
                isOpen={generarPDF.isModalOpen}
                    ...
                    <div className="flex flex-col gap-6 p-4">
                        <div className="overflow-hidden rounded-xl border border-teal-100 shadow-sm">
                            <table className="w-full text-left text-[11px] uppercase">
                                ...
                                <tbody>
                                    {generarPDF.formatosPago.length > 0 ? (
                                        generarPDF.formatosPago.map((f) => (
                                            <tr key={f.id} className="border-t border-teal-50 hover:bg-teal-50/30 transition-colors">
                                                ...
                                                <td className="p-3">
                                                    <span className={`px-2 py-1 rounded-full text-[9px] ${
                                                        f.estadoFirma === 'firmado' ? 'bg-green-100 text-green-600' :
                                                        f.estadoFirma === 'escaneado' ? 'bg-blue-100 text-blue-600' :
                                                        'bg-yellow-100 text-yellow-600'
                                                    }`}>
                                                        {f.estadoFirma === 'firmado' ? 'FIRMADO' : 
                                                        f.estadoFirma === 'escaneado' ? 'ES-CANEADO' : 'PENDIENTE'}
                                                    </span>
                                                </td>
                        ...
                        <button
                            onClick={() => generarPDF.cambiarVista('formulario')}
                            className="bg-teal-600 text-white py-3 rounded-full font-bold uppercase text-[10px] shadow-lg hover:bg-teal-700 transition-all"
                        >
                            + Generar Nuevo Formato de Pago
                        </button>
                    </div>
                ) : generarPDF.vista === 'formulario' ? (
                    <FormularioGenerarPagoPDF
                        pagoSeleccionado={null}
                        trabajadores={trabajadores.trabajadoresActivos}
                        trabajosRealizados={trabajo.listaTrabajos}
                        onGenerarPDF={generarPDF.generarFormatoPago}
                        onCancelar={() => generarPDF.cambiarVista('lista')}
                    />
                ) : (
